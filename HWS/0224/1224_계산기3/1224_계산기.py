...
for tc in range(1,11):
    ...
    for i in data:
        ...
        # print(lst)
            ...
            while stack and (stack[-1]=='*' or stack[-1]=='/'): #곱하기 나누기 우선순위가 서로 같을때
                lst+=stack.pop()
            stack.append(i)
        elif i=='+'or i=='-':
            while stack and (stack[-1]=='*' or stack[-1]=='/'or stack[-1]=='+'or stack[-1]=='-'): # 이거는 우선순위가 가장낮은 '('괄호가 아닐때랑 같음
                lst+=stack.pop()
            stack.append(i)
        elif i=='(': #여는괄호는 닥넣기
            stack.append(i)
        elif i==')':
            while stack and stack[-1]!='(': #닫는괄호나오면 여는괄호 나올때까지 pop
                lst+=stack.pop()
            stack.pop()#마지막에 남는 여는괄호 pop
